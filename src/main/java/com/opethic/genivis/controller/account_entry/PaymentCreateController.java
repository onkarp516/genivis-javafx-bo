...
public class PaymentCreateController implements Initializable {
        ...
        /*tfTranxDate.setOnKeyPressed(new EventHandler<KeyEvent>() {
            ...
            public void handle(KeyEvent keyEvent) {
                ...
                if (keyEvent.getCode() == KeyCode.TAB || keyEvent.getCode() == KeyCode.ENTER) {
                    tblvPaymentCreate.getFocusModel().focus(1, tblcPaymentCreateParticulars);
                    tblvPaymentCreate.requestFocus();
                    tblvPaymentCreate.edit(0, tblcPaymentCreateParticulars);
    ...
    public void setPurChallDataToProduct() {
        ...
        String voucherNo = tfpaymentCreateVoucherSrNo.getText();
        String receiptNo = tfpaymentCreateVoucherNo.getText();
        String narration = tfPaymentNarration.getText();

        PurTranxToProductRedirectionDTO mData = new PurTranxToProductRedirectionDTO();
        mData.setRedirect(FxmFileConstants.TRANX_PAYMENT_CREATE_SLUG);
        ...
        List<PaymentRowDTO> currentItems = new ArrayList<>(tblvPaymentCreate.getItems());
...
//        }
    ...
    public void setRedirectData(Object inObj) {
        ...
        int cnt = 0, index = 0;
        tfTranxDate.setText(franchiseDTO.getTranxDate());
        tfpaymentCreateVoucherSrNo.setText(franchiseDTO.getPurSerNum());
        tfpaymentCreateVoucherNo.setText(franchiseDTO.getChallanNo());
...
//        tblvPaymentCreate.getSelectionModel().getSelectedItem().setParticulars("VINIT");
//        tblvPaymentCreate.getItems().get(0).setParticulars("VINIT");
        ...
        for (PaymentRowDTO purchaseInvoiceTable : franchiseDTO.getRowReceipt()) {
            ...
            //    purchaseInvoiceTable.setParticulars("VINIT");
                purchaseInvoiceTable.setCredit("");
                purchaseInvoiceTable.setDebit("");
            }
            tblvPaymentCreate.getItems().addAll(purchaseInvoiceTable);
            cnt++;
            index++;
        }

        Platform.runLater(()->{
            TableColumn<PaymentRowDTO, ?> colName = tblvPaymentCreate.getColumns().get(1);
            tblvPaymentCreate.edit(redirectIndex, colName);
        });
...
//        btnPaymentList.setText("List");
    ...
    private void moveFocusToNextCell(TablePosition<PaymentRowDTO, String> position, boolean nextColumn) {
        ...
        System.out.println("nextCol->" + nextColumn);
        if (nextColumn) {
    ...
    private void tableInitialization() {
        ...
        tblcPaymentCreateParticulars.setCellFactory(column -> new TextFieldTableCellForPayment("particular", bpRootPane, callback,addPrdCalbak));
...
//        labelPaymentCreateTotalDebit.setText(String.valueOf(totalDebit));
        ...
        labelPaymentCreateTotalCredit.setText(String.valueOf(creditTotal));
        double debitTotal=GlobalTranx.TranxRoundDigit(totalDebit, GlobalTranx.configDecimalPlace);
        labelPaymentCreateTotalDebit.setText(String.valueOf(debitTotal));
            ...
            if (number == 1) {
                ...
                try {
                    ...
                    if (btnText.equalsIgnoreCase("Submit")) {
                        ...
                        //? HIGHLIGHT
                        PaymentListController.isNewPaymentCreated = true; //? Set the flag for new creation
                    } else {
                        apiClient = new APIClient(EndPoints.UPDATE_PAYMENT_ENDPOINT, finalReq, RequestType.FORM_DATA);
                        //? HIGHLIGHT
                        PaymentListController.editedPaymentId = paymentInvoiceId.toString(); //? Set the ID for editing
                    }
...
//                        /**** getting Bills List from selected Table Row if Available--> Edit Case *****/
                        ...
                        /**** getting Bank Details from selected Table Row if Available--> Edit Case *****/
...
//                            rowSelectedAmt = Double.parseDouble(getTableView().getItems().get(getIndex()-1).getDebit());
            ...
            if (columnName.equalsIgnoreCase("particular")) {
                SingleInputDialogs.openLedgerPopUp(stage, "Creditors", input -> {
                    ...
                    if (mBillRow.size() > 0) {
                        for (BillByBillRowReqDTO mRowDTO : mBillRow) {
                            selectedBills.addAll(mRowDTO.getPerticulars().getBillids());
                            System.out.println("Payment Type:::::" + mRowDTO.getPaymentType());
                        }
    ...
    public TextFieldTableCellForPayment(String columnName, Parent rootPane, TableCellCallback<String[]> callback,TableCellCallback<Object[]> addPrdCalbak) {
    ...
    private void getLedgerPendingBills(List<BillByBillId> selectedBills, Stage stage, String ledgerId,
        ...
        /**** Bank Account ****/
            ...
            openBankAccountPopup(stage, ledgerId, ledgerType, balancingMethod, ledgerName, underSlug, "Bank Account", (billParticular, rowData) -> {
                ...
                System.out.println("Payment Type Bill Row:" + rowData.get(0));
            ...
            apiClient.setOnSucceeded(new EventHandler<WorkerStateEvent>() {
                ...
                public void handle(WorkerStateEvent workerStateEvent) {
                    ...
                    if (pendingBillResDTO.getResponseStatus() == 200) {
                        ...
                        lblBalance.setText("" + GlobalTranx.TranxRoundDigit(totalBalance, 2));
                        lblBalanceDN.setText("" + GlobalTranx.TranxRoundDigit(totalBalanceDn, 2));
                        lblInvoiceBalance.setText("" + GlobalTranx.TranxRoundDigit(totalBalanceAmt, 2));
                        lblInvoiceBalanceDN.setText("" + GlobalTranx.TranxRoundDigit(totalBalanceAmtDn, 2));
                        /***** Update case *****/
                        if (selectedBills.size() > 0) {
