...
public class LedgerCreateOtherController implements Initializable {

    @FXML
    private Button btnLedgerCreateOtherSubmit;

    @FXML
    private Button btnLedgerCreateOtherCancel;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        btnLedgerCreateOtherSubmit.setOnAction(actionEvent -> {

            globalLedgerCreateController.fcCallLedgerCreateOtherSubmit();

        });
        btnLedgerCreateOtherCancel.setOnAction(actionEvent -> {
            globalLedgerCreateController.fcCallLedgerCreateOtherCancel();

        });

        btnLedgerCreateOtherSubmit.setOnKeyPressed(event->{
            if(event.getCode() == KeyCode.RIGHT){
                btnLedgerCreateOtherCancel.requestFocus();
            }
        });
    }
