...
public class LedgerCreateAssetsController implements Initializable {

    @FXML
    private Button btnLedgerCreateAssetsSubmit;

    @FXML
    private Button btnLedgerCreateAssetsCancel;
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        btnLedgerCreateAssetsSubmit.setOnAction(actionEvent -> {
            globalLedgerCreateController.fcCallLedgerCreateOtherSubmit();
        });
        btnLedgerCreateAssetsCancel.setOnAction(actionEvent -> {
            globalLedgerCreateController.fcCallLedgerCreateOtherCancel();
        });

        btnLedgerCreateAssetsSubmit.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.RIGHT) {
                btnLedgerCreateAssetsCancel.requestFocus();
            }
        });
