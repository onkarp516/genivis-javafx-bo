...
import com.opethic.genivis.controller.tranx_sales.common.ComboBoxTableCellForSalesInvoice;
...
import com.opethic.genivis.dto.reqres.tranx.sales.invoice.TranxSalesInvoiceRtnLastRecordRes;
...
import static com.opethic.genivis.utils.FxmFileConstants.SALES_INVOICE_RTN_LIST_SLUG;

public class TranxSalesInvoiceRtnCreate implements Initializable {
    ...
    @FXML
    private TableColumn<TranxRow, String> tcTranxParticulars;
    ...
    private static final Logger loggerTranxSalesReturnCreate = LogManager.getLogger(TranxSalesInvoiceRtnCreate.class);
    ...
    private TranxSalesInvoiceLedgerWiseTbl selectedSalesInvoiceId;
    private List<TranxRowRes> selectedInvoiceProducts;
...
//        hlSaleInvCreateChallan.setText(0 + "");
        ...
        initDiscLedger();
//        initHL();
//        nodetraversal(dpTranxSalesInvoiceCreateInvoiceDate, tfTranxSalesInvoiceCreateLedgerName);
...
////        nodetraversal(hlSaleInvCreateChallan, tblTranxSalesInvoiceCreateCMPTRow);
//        cmbSaleInvCreateDeliveryType.setOnKeyPressed((e) -> {
//            if (e.getCode() == KeyCode.ENTER) {
//                tblTranxSalesInvoiceCreateCMPTRow.edit(0, tcTranxParticulars);
//            }
//            e.consume();
//        });
...
//            e.consume();
//        });
        GlobalTranx.requestFocusOrDieTrying(tfTranxSalesInvoiceCreateLedgerName, 3);
        initGstTable();
    }
...
//                    System.out.println("sales challan input => " + input);
    ...
    private void getPendingSalesChallanWithids() {
        List<TranxRowConvRes> lstRowRes = TranxCommonPopUps.getPendingSalesChallansWithIds(referenceSCId);
        setRowConvData(lstRowRes);
    }
...
//                e.printStackTrace();
                loggerTranxSalesReturnCreate.error("Exception in setRowConvData() :" + Globals.getExceptionString(e));
            }
        }
        TranxCal();
    }
    ...
    private void initSaleLastInvoiceNo() {
        ...
        HttpResponse<String> response = APIClient.getRequest(EndPoints.TRANX_SALES_INVOICE_RTN_LAST_RECORD);
        TranxSalesInvoiceRtnLastRecordRes tranxSalesInvoiceLastRecordRes = new Gson().fromJson(response.body(), TranxSalesInvoiceRtnLastRecordRes.class);
        if (tranxSalesInvoiceLastRecordRes.getResponseStatus() == 200) {
            tfTranxSalesInvoiceCreateInvoiceNo.setText(tranxSalesInvoiceLastRecordRes.getCreditnoteNo());
        ...
        try {
            ...
            if (jsonObject.get("responseStatus").getAsInt() == 200) {
                ...
                if (responseObject.size() > 0) {
                    ...
                    loggerTranxSalesReturnCreate.info("initSalesAcc(): ResponseObject is null ");
                }
            } else {
                loggerTranxSalesReturnCreate.error("Exception on initSalesAcc(): ");
            }
        } catch (Exception e) {
            loggerTranxSalesReturnCreate.error("Exception on initSalesAcc(): " + Globals.getExceptionString(e));
        }
    }

    private void handleTfLedgerName() {
        stage = (Stage) rootAnchorPane.getScene().getWindow();
        SingleInputDialogs.openSalesLedgerWithNamePopUp(stage, "Filter", ledgerName, input -> {
            loggerTranxSalesReturnCreate.debug("Callback to Get all the Details of the Selected ledger and Set it in the Fields", input);
            ...
            loggerTranxSalesReturnCreate.debug("Opened Ledger Bill Modal:On Clicking by selecting the Ledger Name ");
            TranxCal();
            tranxLedgerDetailsFun(ledgerId);
            handleSalesInvoiceBills();
        });
        cmbTranxSalesInvoiceCreateSupplierGST.getSelectionModel().selectedItemProperty().addListener((obs, old, nw) -> {
            TranxCal();
        });
    }

    private void handleSalesInvoiceBills() {
        stage = (Stage) rootAnchorPane.getScene().getWindow();
        TranxCommonPopUps.openSalesInvoiceList(stage, ledgerId, "Invoice Bills", input -> {
            selectedSalesInvoiceId = input;
            if (input != null) {
                handleSalesInvoiceProducts();
            }
        });
    }

    private void handleSalesInvoiceProducts() {
        stage = (Stage) rootAnchorPane.getScene().getWindow();
        TranxCommonPopUps.openSalesInvoiceProductList(stage, selectedSalesInvoiceId.getId(), "Bill Products", input -> {
            selectedInvoiceProducts = input;
            setRowReturnData(input);
        });
    }

    private void setRowReturnData(List<TranxRowRes> lstInput) {
        tblTranxSalesInvoiceCreateCMPTRow.getItems().clear();
        for (TranxRowRes currRowRes : lstInput) {
            try {
                TranxSelectedProduct selectedProduct = TranxCommonPopUps.getSelectedProductFromProductId(currRowRes.getProductName());
                TranxRow currRow = GlobalTranx.newEmptyTranxRow();
                currRow.setDetailId(0);
                currRow.setProductId(currRowRes.getProductId());
                currRow.setProductName(currRowRes.getProductName());
                ...
                e.printStackTrace();
                loggerTranxSalesReturnCreate.error("Exception in setRowData() :" + Globals.getExceptionString(e));
            }
        }
        TranxCal();
    }
...
//        tcTranxParticulars.setCellFactory(TextFieldTableCell.forTableColumn());
        ...
        tcTranxQty.setCellFactory(TextFieldTableCellForSalesInvoiceWithConverter.forTableColumn(new IntegerStringConverter()));
        ...
        tcTranxFreeQty.setCellFactory(TextFieldTableCellForSalesInvoiceWithConverter.forTableColumn(new IntegerStringConverter()));
        ...
        tcTranxRate.setCellFactory(TextFieldTableCellForSalesInvoiceWithConverter.forTableColumn(new DoubleStringConverter()));
        ...
        tcTranx1DisPer.setCellFactory(TextFieldTableCellForSalesInvoiceWithConverter.forTableColumn(new DoubleStringConverter()));
        ...
        tcTranx2DisPer.setCellFactory(TextFieldTableCellForSalesInvoiceWithConverter.forTableColumn(new DoubleStringConverter()));
        ...
        tcTranxDisAmt.setCellFactory(TextFieldTableCellForSalesInvoiceWithConverter.forTableColumn(new DoubleStringConverter()));
...
//                private final Button edtButton = new Button("", edtImg);
                ...
                {
                    actionButton.setMaxHeight(38);
                    actionButton.setPrefHeight(38);
                    actionButton.setMinHeight(38);
                    actionButton.setPrefWidth(35);
                    actionButton.setMaxWidth(35);
                    actionButton.setMinWidth(35);
            ...
            public void handle(KeyEvent event) {
                if (event.getCode() == KeyCode.TAB && event.isShiftDown()) {
                    ...
                    if (focusedCell != null) {
                        int columnIndex = focusedCell.getColumn();
                        if (columnIndex > 0) {
                            TableColumn<TranxRow, ?> previousColumn = tblTranxSalesInvoiceCreateCMPTRow.getVisibleLeafColumns().get(columnIndex);
                            String actValue = String.valueOf(previousColumn.getCellObservableValue(focusModel.getFocusedIndex()).getValue());
                            if (actValue.isEmpty())
                                GlobalTranx.requestFocusOrDieTrying(tfTranxSaleInvCreateNarration, 3);
                        }
                    }
                }
                event.consume();
                    ...
                    case "tcTranxParticulars":
                        TranxCommonPopUps.openProductPopUp(stage, selectedRow.getProductId(), "Product", input -> {
                            ...
                            if (input != null) {
                                ...
                                tblTranxSalesInvoiceCreateCMPTRow.getFocusModel().focus(row, tcTranxUnit);
                                tblTranxSalesInvoiceCreateCMPTRow.edit(row, tcTranxUnit);
                                ...
                                tblTranxSalesInvoiceCreateCMPTRow.getFocusModel().focus(row, tcTranxParticulars);
                                tblTranxSalesInvoiceCreateCMPTRow.edit(row, tcTranxParticulars);
                            }
                        });
                        break;
                    case "tcTranxBatchNo":
...
//                                    selectedRow.setRate(input.minRateA);
                                ...
                                tblTranxSalesInvoiceCreateCMPTRow.edit(row, focuscolumn);
                ...
                loggerTranxSalesReturnCreate.error("Exception in getFocusModel(): " + Globals.getExceptionString(e));
            }
        });
        tblTranxSalesInvoiceCreateCMPTRow.getItems().clear();
        tblTranxSalesInvoiceCreateCMPTRow.getItems().add(GlobalTranx.newEmptyTranxRow());
...
//            System.out.println("cal ->" + cal);
            tbColumnFocus = visibleColumns.get(cal);
...
//            System.out.println("tbColumnFocus -> " + tbColumnFocus.getText());
    ...
    private TableCell<TranxRow, String> getCellUnits(TableColumn<TranxRow, String> tableColumn) {
        return new ComboBoxTableCellForSalesInvoice<>() {
            @Override
    ...
    private void callCreateSalesInvoiceAPI() {
        ...
        for (int i = 0; i < tblTranxSalesInvoiceCreateCMPTRow.getItems().size(); i++) {
            ...
            if (!tRow.getProductName().isEmpty()) {
                ...
                jsonObject.put("b_details_id", tRow.getSelectedBatch()!=null?tRow.getSelectedBatch().getId():0);
...
//                jsonObject.put("manufacturing_date", tRow.getSelectedBatch().getManufacturingDate());
        ...
        body.put("sales_invoice_id", String.valueOf(selectedSalesInvoiceId.getId()));
        body.put("source", selectedSalesInvoiceId.getSource());
        ...
        body.put("tcs_mode", "");
        body.put("sales_discount", String.valueOf(tfTranxSaleInvCreateDisPer.getText()));
        body.put("total_sales_discount_amt", String.valueOf(tfTranxDiscountTotal.getText()));
        body.put("sales_discount_amt", String.valueOf(tfTranxSaleInvCreateDisAmt.getText()));
        body.put("row", String.valueOf(rowArr));
        body.put("additionalChargesTotal", tfTranxAdditionalTotal.getText().isEmpty() ? "0.0" : tfTranxAdditionalTotal.getText());
        ...
        if (referenceSCId.size() > 0) {
            JSONArray reqReferenceSCIds = new JSONArray();
            for (Integer i : referenceSCId) {
                JSONObject obj = new JSONObject();
                obj.put("id", i);
                reqReferenceSCIds.put(obj);
            }
            body.put("reference_sc_id", reqReferenceSCIds.toString());
            body.put("reference", "SLSCHN");
        }
        if (referenceSOId.size() > 0) {
            JSONArray reqReferenceSOIds = new JSONArray();
            for (Integer i : referenceSOId) {
                JSONObject obj = new JSONObject();
                obj.put("id", i);
                reqReferenceSOIds.put(obj);
            }
            body.put("reference_so_id", reqReferenceSOIds.toString());
            body.put("reference", "SLSORD");
        }
        if (referenceSQId.size() > 0) {
            JSONArray reqReferenceSQIds = new JSONArray();
            for (Integer i : referenceSQId) {
                JSONObject obj = new JSONObject();
                obj.put("id", i);
                reqReferenceSQIds.put(obj);
            }
            body.put("reference_sq_id", reqReferenceSQIds.toString());
            body.put("reference", "SLSQTN");
        }

        Map<String, String> headers = new HashMap<>();
//        headers.put("branch", "gvhm001");
        String response = APIClient.postMultipartRequest(body, null, EndPoints.TRANX_SALES_RETURN_CREATE, headers);
    ...
    public void TranxCancel(ActionEvent actionEvent) {
        AlertUtility.AlertConfirmation(AlertUtility.alertTypeConfirmation, "Are you sure?", input -> {
            if (input == 1) initSalesInvoiceForm();
        });
    }
        ...
        current_node.setOnKeyPressed(event -> {
            if (event.getCode() == KeyCode.ENTER) {
                ...
                if (current_node instanceof TextField textField) {
                    if (textField.getId().equals("tfTranxSalesInvoiceCreateLedgerName")) {
                        handleTfLedgerName();
                    }
                }
...
//            System.out.println(addChargesDTOList);
    ...
    private void tranxLedgerDetailsFun(String id) {
        ...
        loggerTranxSalesReturnCreate.debug("Get Ledger Details Data Started...");
...
//            txtPurOrderTransportName.setText(jsonObject.get("contact_name").getAsString());
        ...
        apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent workerStateEvent) {
                loggerTranxSalesReturnCreate.error("Network API cancelled in tranxLedgerDetailsFun()" + workerStateEvent.getSource().getValue().toString());
        ...
        apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent workerStateEvent) {
                loggerTranxSalesReturnCreate.error("Network API failed in tranxLedgerDetailsFun()" + workerStateEvent.getSource().getValue().toString());
            }
        });
        apiClient.start();
        loggerTranxSalesReturnCreate.debug("Get Ledger Details Data End...");
    ...
    private void tranxProductDetailsFun(String id) {
        ...
        loggerTranxSalesReturnCreate.debug("Get Product Details Data Started...");
        Map<String, String> map = new HashMap<>();
...
//                System.out.println("tranxProductDetailsFun Res ==>" + jsonObject);
        ...
        apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent workerStateEvent) {
                loggerTranxSalesReturnCreate.error("Network API cancelled in tranxProductDetailsFun()" + workerStateEvent.getSource().getValue().toString());
        ...
        apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent workerStateEvent) {
                loggerTranxSalesReturnCreate.error("Network API failed in tranxProductDetailsFun()" + workerStateEvent.getSource().getValue().toString());
            }
        });
        apiClient.start();
        loggerTranxSalesReturnCreate.debug("Get Product Details Data End...");
    ...
    private void getSupplierListbyProductId(String id) {
        APIClient apiClient = null;
        loggerTranxSalesReturnCreate.debug("Get Supplier List Data Started...");
        Map<String, String> map = new HashMap<>();
...
//            JsonObject resultObj = jsonObject.getAsJsonObject("result");
        ...
        apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent workerStateEvent) {
                loggerTranxSalesReturnCreate.error("Network API cancelled in getSupplierListbyProductId()" + workerStateEvent.getSource().getValue().toString());
        ...
        apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
            @Override
            public void handle(WorkerStateEvent workerStateEvent) {
                loggerTranxSalesReturnCreate.error("Network API failed in getSupplierListbyProductId()" + workerStateEvent.getSource().getValue().toString());
            }
        });
        apiClient.start();
        loggerTranxSalesReturnCreate.debug("Get Supplier List Data End...");
