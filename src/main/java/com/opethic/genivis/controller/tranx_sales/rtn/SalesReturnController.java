...
import com.opethic.genivis.controller.tranx_purchase.PurchaseReturnListController;
...
public class SalesReturnController implements Initializable {
    ...
    private static final Logger salesRtnLogger = LoggerFactory.getLogger(SalesReturnController.class);
...
//        redirectToListScreen();
        ...
        salesRtnLogger.debug("Opened Ledger Modal:On Clicking on by selecting Ledger Name and " + "Select the Ledger from the Modal");
        tfPurRtnLedgName.setOnMouseClicked(actionEvent -> {
            ...
            SingleInputDialogs.openLedgerPopUp(stage, "Debtors", input -> {
                        salesRtnLogger.debug("Callback to Get all the Details of the Selected ledger and Set it in the Fields");
                        ...
                        System.out.println("Ledger StateCode:"+ledgerStateCode);
                        ...
                        salesRtnLogger.debug("Bill by Bill by Window --> Opened Ledger Bill Modal:On Clicking by " + "selecting the Ledger Name ");
                    },
                    input -> {
                        if (input == true) {
                            isLedgerRed = true;
        ...
        tfPurRtnLedgName.setOnKeyPressed(ev -> {
            if (ev.getCode() == KeyCode.SPACE || ev.getCode() == KeyCode.ENTER) {
                try {
                    ...
                    salesRtnLogger.error("Exception in opening OpenLedgerPopup():" + exceptionAsString);
                }
            }
    ...
    private void getBillbyBillWindow(Stage stage, String ledgerId) {
        SingleInputDialogs.openInvoiceListSalesRtn(Long.parseLong(ledgerId), stage, "Select Bill", input -> {
            salesRtnLogger.debug("Callback to Get all the Details of the Selected ledger and Set it in the Fields");
    ...
    private void getTranxRtnProductList(Stage stage, String billId) {
        ...
        SingleInputDialogs.openTranxRtnProductList(billId, stage, "Select Products","srtn", (input1, input2) -> {

            salesRtnLogger.debug("Callback to get all the Tranx Sales product details of the selected invoice number");
            ...
            if (input1.size() > 0) {
                for (TranxPurRowResEditDTO mList : input1) {
                    salesRtnLogger.debug("For " + mList.getProductName() + " setting up the row value started");
                    ...
                    try {
                        ...
                        SalesInvoiceCalculationRtn.rowCalculationForPurcInvoice(index, tvPurInvCreateCMPTRow, callback);
                        SalesInvoiceCalculationRtn.calculateGst(tvPurInvCreateCMPTRow, callback);
                        List<LevelAForPurInvoice> levelAForPurInvoiceList = SaleProductUnitsPackingRtn.getAllSaleProductUnitsPackingRtnFlavour(newRow.getProduct_id());
                        ...
                        index++;
                        salesRtnLogger.debug("For " + mList.getProductName() + " setting up the row value ended");
...
//            scBottomMain.setPrefHeight(150);
    ...
    private void validateInvoiceDate() {
        try {
            salesRtnLogger.debug("Initiate the validateInvoiceDate() method");
            ...
            if (obj.get("responseStatus").getAsInt() == 404) {
                AlertUtility.AlertWarningTimeout(AlertUtility.alertTypeWarning, obj.get("message").getAsString(), callback -> {
                    salesRtnLogger.debug("Invoice Date : " + invoiceDate + " not in the Financial Year in Pur Return");
                    salesRtnLogger.debug("END of validateInvoiceDate() method in Pur Return");
                });
                tfPurRtnInvDate.requestFocus();
            }
        } catch (Exception e) {
            ...
            e.printStackTrace(new PrintWriter(sw));
            String exceptionAsString = sw.toString();
            salesRtnLogger.error("Exception in validateInvoiceDate:" + exceptionAsString);
        }
        ...
        if (purRtnEditId > 0) {
            ...
            if (!TextUtils.isEmpty(discText)) {
                ...
                for (int i = 0; i < tvPurInvCreateCMPTRow.getItems().size(); i++) {
                    ...
                    SalesInvoiceCalculationRtn.rowCalculationForPurcInvoice(i, tvPurInvCreateCMPTRow, callback);//call row calculation function
                ...
                SalesInvoiceCalculationRtn.discountPropotionalCalculation(disc_per + "", "0", tfPurchaseRtnAddChgAmt.getText(), tvPurInvCreateCMPTRow, callback);
                if (!tfPurchaseRtnAddChgAmt.getText().isEmpty()) {
                    System.out.println("Additioanal CHarges Called >> ");
                    SalesInvoiceCalculationRtn.additionalChargesCalculation(tfPurchaseRtnAddChgAmt.getText(), tvPurInvCreateCMPTRow, callback);
                }
            } else {
                tfPurRtnDisAmt.setText("");
                SalesInvoiceCalculationRtn.discountPropotionalCalculation("0", "0", tfPurchaseRtnAddChgAmt.getText(), tvPurInvCreateCMPTRow, callback);
                if (!tfPurchaseRtnAddChgAmt.getText().isEmpty()) {
                    System.out.println("Additioanal CHarges Called >> ");
                    SalesInvoiceCalculationRtn.additionalChargesCalculation(tfPurchaseRtnAddChgAmt.getText(), tvPurInvCreateCMPTRow, callback);
                }
...
//                calculateGst(tableView, callback);
                ...
                List<LevelAForPurInvoice> levelAForPurInvoiceList = SaleProductUnitsPackingRtn.getAllSaleProductUnitsPackingRtnFlavour(purchaseInvoiceTable.getProduct_id());
...
//                    purchaseInvoiceTable.setTotal_taxable_amt(String.valueOf(orgTaxableAmt));
    ...
    public void purc_disc_amt(KeyEvent keyEvent) {
        ...
        if (!TextUtils.isEmpty(discAmtText)) {
            ...
            SalesInvoiceCalculationRtn.discountPropotionalCalculation("0", disc_amt + "", tfPurchaseRtnAddChgAmt.getText(), tvPurInvCreateCMPTRow, callback);
            if (!tfPurchaseRtnAddChgAmt.getText().isEmpty()) {
                System.out.println("Additioanal CHarges Called >> ");
                SalesInvoiceCalculationRtn.additionalChargesCalculation(tfPurchaseRtnAddChgAmt.getText(), tvPurInvCreateCMPTRow, callback);
            }
    ...
    public void onClickAdditionalCharges() {
        ...
        AdditionalCharges.additionalCharges(stage, addChargesDTOList, total_taxable_amt, (input1, input2) -> {
            ...
            SalesInvoiceCalculationRtn.additionalChargesCalculation(input1[1], tvPurInvCreateCMPTRow, callback);
    ...
    private void getPurRtnSrNumber() {
        ...
        try {
            salesRtnLogger.debug("Get getPurRtnSrNumber Started...");
            ...
            apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API cancelled in getPurRtnSrNumber()" + workerStateEvent.getSource().getValue().toString());
            ...
            apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API failed in getPurRtnSrNumber()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.start();
            salesRtnLogger.debug("Get getPurRtnSrNumber End...");
        } catch (Exception e) {
            StringWriter sw = new StringWriter();
            e.printStackTrace(new PrintWriter(sw));
            String exceptionAsString = sw.toString();
            salesRtnLogger.error("Exception in getVoucherNumber():" + exceptionAsString);
        }
    ...
    public void getPurchaseAccounts() {
        APIClient apiClient = null;
        try {
            salesRtnLogger.debug("Get getPurchaseAccounts Started...");
                ...
                public void handle(WorkerStateEvent workerStateEvent) {
                    ...
                    if (jsonObject.get("responseStatus").getAsInt() == 200) {
                        ...
                        if (responseObject.size() > 0) {
                            ...
                            salesRtnLogger.debug("Get getPurchaseAccounts responseObject is null...");
                            System.out.println("responseObject is null");
                        }
                    } else {
                        salesRtnLogger.debug("Get getPurchaseAccounts Error in response...");
            ...
            apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API cancelled in getPurchaseAccounts()" + workerStateEvent.getSource().getValue().toString());
            ...
            apiClient.start();
            salesRtnLogger.debug("Get getPurchaseAccounts  end");
...
//                    creditNoteLogger.error("Network API failed in createCreditNote()" + workerStateEvent.getSource().getValue().toString());
    ...
    private void getPurRtnById(Integer purRtnEditId) {
        try {
            ...
            e.printStackTrace(new PrintWriter(sw));
            String exceptionAsString = sw.toString();
            salesRtnLogger.error("Exception in getPurRtnById():" + exceptionAsString);
        }
    }
    ...
    private void setPurRtnFormData(PurReturnEditDTO responseBody) {
        try {
            ...
            e.printStackTrace(new PrintWriter(sw));
            String exceptionAsString = sw.toString();
            salesRtnLogger.error("Exception in setPurRtnFormData():" + exceptionAsString);
        }
    }
    ...
    public String ledgersById(Long id) {
        try {
            ...
            e.printStackTrace(new PrintWriter(sw));
            String exceptionAsString = sw.toString();
            salesRtnLogger.error("Exception in ledgersById():" + exceptionAsString);
        }
        return ledgerName;
    }
    ...
    private void setAddChargesInTable() {
        ...
        SalesInvoiceCalculationRtn.additionalChargesCalculation(totalAddChgAmt.toString(), tvPurInvCreateCMPTRow, callback);
...
//                    System.out.println("jsonObject Res ==>" + jsonObject);
            ...
            apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API cancelled in tranxProductDetailsFun()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API failed in tranxProductDetailsFun()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.start();
        } catch (Exception e) {
...
//                        System.out.println("jsonObj " + jsonObj);
            ...
            apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API cancelled in tranxLedgerDetailsFun()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API failed in tranxLedgerDetailsFun()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.start();
        } catch (Exception e) {
    ...
    private void getSupplierListbyProductId(String id) {
        ...
        try {
            ...
            apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API cancelled in getSupplierListbyProductId()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API failed in getSupplierListbyProductId()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.start();
        } catch (Exception e) {
    ...
    private void tranxBatchDetailsFun(String bNo, String bId) {
        ...
        salesRtnLogger.debug("Get Supplier List Data Started...");
...
//                System.out.println("jsonObject =>" + jsonObject);
        ...
        apiClient.start();
        salesRtnLogger.debug("Get Supplier List Data End...");
    ...
    public void validatePurchaseReturn() {
        ...
        try {
            ...
            apiClient.setOnCancelled(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API cancelled in functionCompanyDuplicate()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.setOnFailed(new EventHandler<WorkerStateEvent>() {
                @Override
                public void handle(WorkerStateEvent workerStateEvent) {
                    salesRtnLogger.error("Network API failed in functionCompanyDuplicate()" + workerStateEvent.getSource().getValue().toString());
                }
            });
            apiClient.start();
        } catch (Exception e) {
...
//                    getTableView().getItems().addAll(new PurchaseInvoiceTable("", String.valueOf(current_index + 1), String.valueOf(serial_no + 1), "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""));
    ...
    private void openProduct() {
        ...
        SingleInputDialogs.openProductPopUpForPurchase(Communicator.stage, getTableView().getItems().get(getIndex()).getParticulars(), "Product", input -> {
            ...
            List<LevelAForPurInvoice> levelAForPurInvoiceList = SaleProductUnitsPackingRtn.getAllSaleProductUnitsPackingRtnFlavour(productId);
    ...
    public void openBatchWindow() {
        ...
        if (current_table_row.getIs_batch().equals("true")) {
            BatchWindow.batchWindow(String.valueOf(current_index), product_details_to_batch_window, batchWindowTableDTOList, input -> {
                ...
                SalesInvoiceCalculationRtn.rowCalculationForPurcInvoice(Integer.parseInt(input.get(0).getCurrentIndex()), getTableView(), callback);

                SalesInvoiceCalculationRtn.calculateGst(getTableView(), callback);
...
class SaleProductUnitsPackingRtn {

    public static List<LevelAForPurInvoice> getAllSaleProductUnitsPackingRtnFlavour(String product_id) {
...
class SalesInvoiceCalculationRtn {
