...
public class TranxSalesInvoiceEdit implements Initializable {
    ...
    private static final Logger loggerTranxSalesInvoiceEdit = LogManager.getLogger(TranxSalesInvoiceEdit.class);
...
//        tfTranxSaleInvCreateDisAmt.textProperty().addListener((obs,old,nw)->{
//                TranxCal(false);
//        });
    ...
    public void sceneInitilization() {
        rootAnchorPane.sceneProperty().addListener((observable, oldScene, newScene) -> {
            if (newScene != null && newScene.getWindow() instanceof Stage) {
                ...
                Communicator.tblSaleTranx = (TableView) tblTranxSalesInvoiceCreateCMPTRow;
            }
        });
    }
...
//        if (!Globals.getUserControlsWithSlug("is_free_qty")) ++cnt;
        ...
        List<TableColumn<TranxRow, ?>> visibleColumns = tblTranxSalesInvoiceCreateCMPTRow.getVisibleLeafColumns();
        tbColumnFocus = visibleColumns.get(column + cnt);
...
//                jsonObject.put("manufacturing_date", tRow.getSelectedBatch().getManufacturingDate());
        ...
        body.put("bill_no", tfTranxSalesInvoiceCreateInvoiceNo.getText());
        body.put("sales_acc_id", String.valueOf(cmbTranxSalesInvoiceCreateSalesAcc.getValue().getId()));
        body.put("sales_sr_no", tfTranxSalesInvoiceCreateInvoiceSrNo.getText());
...
//        headers.put("branch", "gvhm001");
        String response = APIClient.postMultipartRequest(body, null, EndPoints.TRANX_SALES_INVOICE_EDIT, headers);
    ...
    public void TranxCancel(ActionEvent actionEvent) {
        AlertUtility.AlertConfirmation(AlertUtility.alertTypeConfirmation, "Are you sure?", input -> {
            if (input == 1) GlobalController.getInstance().addTabStatic(SALES_INVOICE_CREATE_SLUG, false);
            ;
        });
    }
...
//        headers.put("branch", "gvhm001");
        ...
        if (resObj.getInt("responseStatus") == 200) {
            ...
            dpTranxSalesInvoiceCreateInvoiceDate.setValue(DateConvertUtil.convertStringToLocalDate(invoiceData.getString("transaction_dt")));
